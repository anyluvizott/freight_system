...
describe 'Usuário Administrador cadastra um novo valor à tabela' do
  ...
  it 'com valor inicial já inserido na tabela' do
    ...
    expect(page).to have_content '10 kg'
    expect(page).not_to have_content '50 kg'
    expect(page).to have_content 'R$ 0,50'
    expect(page).not_to have_content 'R$ 0,80'
    expect(page).to have_content 'Adicionar Novos Valores à Tabela'
    expect(page).not_to have_content 'Novo valor adicionado'
  end
  ...
  it 'com valor final já inserido na tabela' do
    ...
    first_line = PriceByWeight.create!(starting_weight: 2, final_weight: 10, price_per_km: 0.50)

    login_as(admin)
    visit root_path
    click_on 'Tabela de Preço por Peso'
    fill_in 'Peso Inicial',	with: 1
    fill_in 'Peso Final',	with: 8
    fill_in 'Preço por Km',	with: 0.80
    click_on 'Enviar'
    ...
    expect(page).to have_content '10 kg'
    expect(page).not_to have_content '8 kg'
    expect(page).to have_content 'R$ 0,50'
    expect(page).not_to have_content 'R$ 0,80'
    expect(page).to have_content 'Adicionar Novos Valores à Tabela'
    expect(page).not_to have_content 'Novo valor adicionado'
